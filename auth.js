document.addEventListener('DOMContentLoaded', () => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
        if (session && (window.location.pathname === '/index.html' || window.location.pathname === '/register.html')) {
    ...
    if (registerForm) {
        registerForm.addEventListener('submit', async (e) => {
            ...
            if (error) {
                ...
                Swal.fire('¡Éxito!', 'Revisa tu correo para verificar tu cuenta.', 'success').then(() => {
